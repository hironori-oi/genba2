...
### Phase 4 WORKS+MVP (14 日 / **160 turn** / **二重監査必須**) — **完了 (2026-05-14, deployed without PITR)**
`manufacturing_*` migration、製造画面 (指示 QR→工程→開始/終了→製造数/ロット/設備→不適合 N→登録)、製造入庫同時記録、4 業務統合履歴、MVP 準備 (1 顧客 seed、Vercel 本番投入)。
**Status (2026-05-14)**: **complete (deployed without PITR)** — 本番 URL <https://genba2-ai.vercel.app/> にオーナー手動デプロイ完了。Supabase は **Free tier + 標準日次バックアップ**で運用開始 (RPO 24h を受容)。PITR 採用は **本フェーズで見送り**、Phase 9 (性能+バックアップ+観測) または顧客数 / トランザクション量 / 観測強化トリガで再評価する (詳細は `docs/RUNBOOK.md` §6 / `docs/SECURITY-AUDIT-2026-05-13-phase4.md` Backup / Disaster Recovery)。
**DoD 達成状況**: 製造 e2e (Phase 4c) / PRODUCT_SPEC §6 全 AC / 本番デプロイ (オーナー手動) / **security-auditor conditional pass (P0=0 P1=0 P2=2)** / RUNBOOK §0/§3.3/§6/§7 反映済 / production smoke (`/` `/login` `/app/logi` `/app/works/manufacturing` で 200/200/307/307、5xx=0)。**唯一の DoD 変更点**: 「PITR 有効」→「PITR は Phase 9 で再評価 (本フェーズではオーナー判断で採用見送り、日次バックアップで運用開始)」。
**リスク**: 不適合 N の UI ページング / 製造入庫二重記録 (`manufacturing_record_id` UNIQUE — Phase 4a partial unique index でクローズ) / k6 軽量検証 (100 同時) は Phase 9 で実施。**新規受容リスク**: 最大 24 時間分のデータ損失 (RPO 24h)、1 顧客 MVP 段階で受容。
...
履歴/`qr_scan_histories` チューニング、Sentry 検討 (**承認必須**) or OTel、月次バックアップ+リストア手順、**PITR 再評価 (Phase 4 で採用見送りを決定済 — 顧客数 / トランザクション量 / 観測強化と合わせて Supabase Pro 切替の要否を判断する。トリガ詳細は `docs/RUNBOOK.md` §6.3)**。
**DoD**: 履歴クエリ < 1.5s (10k 行)、staging リストア成功 (Free tier 日次 snapshot もしくは PITR 切替後の任意時刻リストア)、PITR 採用是非のオーナー判断記録、reviewer pass。
...
**改訂 (2026-05-14, Phase 4d-deploy)**: 上記 Pro+PITR 前倒し方針はオーナー判断で **本フェーズ採用見送り**となった。Phase 4d-deploy では Vercel 本番デプロイ + 環境変数登録のみオーナー手動で実行、Supabase は Free tier (日次バックアップのみ) で運用開始。`paid_subscription_signup` (Pro+PITR) 承認の再起案は Phase 9 (またはトリガ発火時の前倒し dispatch) で実施。受容するリスクは RPO 24h、根拠は `docs/RUNBOOK.md` §6 / `docs/SECURITY-AUDIT-2026-05-13-phase4.md` Backup / Disaster Recovery セクション。
